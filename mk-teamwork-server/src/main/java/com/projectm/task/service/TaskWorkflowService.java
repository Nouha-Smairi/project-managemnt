...
public class TaskWorkflowService extends ServiceImpl<TaskWorkflowMapper, TaskWorkflow> {
    ...
    //Query by item number taskWorkflow
    ...
    //Query taskWorkflowrule based on workflow number
    ...
    public boolean save(String orgCode, String projectCode, String taskWorkflowName, JSONObject rules) {
        ...
        log.info("Task flow save{}，rule save{}", save, saveRules);
        if (save && saveRules) {
            return true;
        } else {
            throw new CustomException("Save failed!");
        }
    }
    ...
    private boolean saveRules(String flowCode, JSONObject rules) {
        ...
        if (StrUtil.isNotEmpty(firstObj)) {
            ...
            log.info("transfer g rules save{}", saveBatch);
            if (!saveBatch) {
                throw new CustomException("Save failed!");
            }
            return true;
        } else {
            throw new CustomException("Parameter error!");
        }
    }
    ...
    public boolean update(String taskWorkflowCode, String taskWorkflowName, JSONObject rules) {
        ...
        log.info("Task flow modification{}，rule removal{}, rule save{}", update, remove, saveRules);
        if (update && remove && saveRules) {
            return true;
        } else {
            throw new CustomException("fail to edit!");
        }
    }
    ...
    public boolean delete(String taskWorkflowCode) {
        ...
        log.info("task removed{}，rule removal{}", remove, removeRule);
    ...
    public void queryRule(String projectCode, String stageCode, String taskCode, String memberCode, Integer action) {
        ...
        //Traverse the rules and make task modifications
        ...
        if (theWorkflow != null) {
            theWorkflow.forEach(o -> {
                ...
                log.info("Modify tasks according to flow rules{}", update);
            });
        }
    }
    private void updateTaskMember(String taskCode, String memberCode) {
            ...
            if (one == null) {
               TaskMember build = TaskMember.builder().task_code(taskCode).member_code(memberCode).is_executor(1).join_time(DateUtil.getCurrentDateTime()).build();
                ...
                log.info("Task member modification{}，Task successfully added{}", update, save);
           } else {
                ...
                log.info("Task member modification{}，member modification{}", update, update1);
            }
       }
