    ...
    <div class="inviteFromLink" style="height: 100%;">
        <div class="content">
            <a-spin :spinning="loading">
                <a-card v-if="inviteLink" :title="`from ${inviteLink.member.name} invitation`">
                    <div class="header">
                    <span>
                        {{inviteLink.member.name}} invite you
                        <span v-if="inviteLink.invite_type == 'project'">join project「{{inviteLink.sourceDetail.name}}」</span>
                        <span v-if="inviteLink.invite_type == 'organization'">join the organization「{{inviteLink.sourceDetail.name}}」</span>
                    ...
                    <a-button type="primary" block size="large" class="middle-btn" @click="acceptInvite">
                        <span v-if="inviteLink.invite_type == 'project'">join project</span>
                        <span v-if="inviteLink.invite_type == 'organization'">join the organization</span>
                    </a-button>
                </a-card>
            </a-spin>
        </div>
