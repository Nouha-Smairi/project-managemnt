    ...
    <div class="member-menu" :class="{'creating': creating,'editing': editing}">
        <a-spin :spinning="listLoading">
            <div class="search-content">
                <a-input v-model="keyword" size="large" :placeholder="(creating || editing) ? 'label name': 'search tags'">
            ...
            <div class="footer">
                <template v-if="creating">
                    <a-button type="default" size="large" @click="creating = false,keyword = ''">Cancel</a-button>
                    <a-button type="primary" :disabled="!keyword" size="large" @click="createTag">create</a-button>
                </template>
                <template v-else-if="editing">
                    <a-popconfirm :overlayStyle="{zIndex: '1080'}" title="Confirm to delete label?" okText="Sure" cancelText="Cancel"
                                  @confirm="deleteTag">
                        <a-button type="danger" size="large">delete</a-button>
                    </a-popconfirm>
                    <a-button type="primary" :disabled="!keyword" size="large" @click="saveTag">Finish</a-button>
                </template>
                <template v-else>
                    <a-button type="primary" size="large" block @click="creating = true,activeCreating = true">new label
                    </a-button>
                </template>
            </div>
        </a-spin>
...
<script>
    ...
    export default {
        ...
        props: {
            ...
            isCommit: {//Whether to submit after selection
                type: [Boolean],
                default() {
                    return true
                }
        ...
        data() {
            return {
                keyword: '',
                editing: false, //Edit tab status
                creating: false, //create tag state
                activeCreating: false,//Whether to manually initiate the creation (when the search result is empty, the creation will also be initiated, but it is not manually initiated)
                searching: false,
                listLoading: false,
        ...
        methods: {
            ...
            search() {
                ...
                if (!this.keyword) {
                    ...
                    if (!this.activeCreating) {//Returns search results if the creation was not manually initiated
                        this.creating = false;
                    }
                }
                this.searching = true;
