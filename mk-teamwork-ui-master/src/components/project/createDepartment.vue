<template>
    <div class="create-department member-menu">
        ...
        <div class="search-content">
            <a-input v-model="name" size="large" :placeholder="`${actionTitle}name`">
                <a-icon slot="prefix" type="bulb" class="muted"/>
            </a-input>
            <div class="info" v-if="parentDepartmentCode">
                <span class="muted">belongs to：{{parentDepartment.name}}</span>
            </div>
        </div>
        <div class="actions">
...
<script>
    ...
    export default {
        ...
        computed: {
            actionType() {
                return this.departmentCode ? 'edit' : 'create'
            },
            actionTitle() {
                return this.parentDepartmentCode ? 'sub-department' : 'department'
            },
            disabled() {
        ...
        methods: {
            ...
            doAction() {
                ...
                doData(obj).then(res => {
                    ...
                    notice({title: `${this.actionType}departmental success`}, 'notice', 'success');
