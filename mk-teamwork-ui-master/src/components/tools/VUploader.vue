    ...
    <div class="v-uploader" :class="{'hidden-files': !showFiles,'hidden-uploader': !showUploader}">
        <a-card :title="uploaderTitle">
            ...
            <uploader ref="uploader"
                ...
                <vue-scroll>
                    <!--<a-button @click="testSomeThing">test</a-button>-->
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                loading: false,
                showFiles: false, //show uploaded files
                showUploader: false,//show upload window
                progressTotal: 0, //Number of files being uploaded
                completeTotal: 0, //completed files
                options: {
                    target: uploadFiles,
                    testChunks: false,
                    query: function () {
                        return getStore('tempData', true);//queryTemporarily unable to dynamically respond
                    },
                    headers: function () {
        ...
        computed: {
            ...
            uploaderTitle() {
                ...
                return `uploading ${current}/${this.progressTotal}`;
            }
        },
        watch: {
            code() {
        ...
        methods: {
            ...
            closeUploader() {//close upload window
                this.showUploader = false;
                this.uploader.cancel();
            },
            filesSubmitted(files) { //add upload file
            ...
            fileProgress(rootFile, file, chunk) { //There is a file uploading
                this.showUploader = true;
                this.showFiles = true;
                console.log('file progress', arguments)
            },
            fileSuccess(rootFile, file, message, chunk) { //A file uploaded successfully
            ...
            fileError(rootFile, file, message, chunk) { //A file upload failed                this.progressTotal--;
                this.completeTotal--;
                const response = JSON.parse(message);
                file.cancel();
                rootFile.projectName = response.data.projectName;
            ...
            fileComplete(rootFile) { //A file upload is complete
                console.log('file complete', rootFile);
            },
            complete() { //All files uploaded
                console.log('complete', arguments);
                this.progressTotal = this.completeTotal = 0;
                notice({title: 'Associated file succeeded'}, 'notice', 'success');
                setTimeout(() => {
