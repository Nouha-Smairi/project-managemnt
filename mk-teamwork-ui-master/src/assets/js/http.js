...
$http.interceptors.request.use(
    config => {
        //When requesting to update the token, other interfaces are waiting
...
$http.interceptors.response.use(
    response => {
        ...
        // if("No permission to operate resource"==response.msg){
        //     debugger;
        // }
        switch (response.code) {
            ...
            case 403:
                notice({
                    title: response.msg !== '' ? response.msg : 'Unauthorized operation resource, access is denied',
                }, 'notice', 'error', 5);
                return Promise.resolve(response);
            case 4031:
                //Unauthorized operation resource
                notice({
                    title: response.msg !== '' ? response.msg : 'Unauthorized operation resource, access is denied',
                }, 'notice', 'error', 5);
            ...
            case 4041:
                ...
                notice({
                    title: response.msg !== '' ? response.msg : 'resource does not exist',
                }, 'notice', 'warning', 5);
                $router.replace(HOME_PAGE);
                return Promise.resolve(response);
        }
        if (response.code === 200) {
            notice({
                title: 'wrong request' + response.code,
                desc: response.msg
            }, 'notice', 'warning', 5);
            return Promise.resolve(response);
        } else {
